      ...
      if (geom.type == "Point") {
        ...
        for (var i = 0; i < geom.coordinates.length; i++) {
          var point = this.Point(
            this.project(geom.coordinates[i], z_coordinate),
            options
          );
          object.geometry = point.geometry;
          object.material = point.material;
...
document.addEventListener("DOMContentLoaded", function (ev) {
  ...
  function requestData(year, month, day) {
    document.body.classList.add("waiting");
    return new Promise(function (res, rej) {
      var url = `/rest/contours/${magnitude}/${year}/${month}/${day}`;
      request(
        url,
        function (geojson) {
          jsonToScene(geojson);
  ...
  Array.apply(
    ...
    el.addEventListener("click", function (ev) {
      ...
      relative = el.dataset.value == "relative";
      jsonToScene(_data);
    });
  });
          ...
          if (val[key] == undefined) {
            ...
            if (key == "year") {
              ...
              if (val[key] < 0) {
                ...
                val["year"] =
                  val["year"] != undefined ? val["year"] - 1 : self.state["year"] - 1;
              } else if (val[key] >= 12) {
                acum[key] = val[key] % 12;
                val["year"] =
                  val["year"] != undefined ? val["year"] + 1 : self.state["year"] + 1;
              ...
              if (val[key] < 0) {
                ...
                val["month"] =
                  val["month"] != undefined ? val["month"] - 1 : self.state["month"] - 1;
...
module.exports = (function () {
  function Calendar(onClick, config) {
    ...
    timelineBody.innerHTML =
      '<div class="calendar__nav backward" scale="month" disabled><abbr title="navegació per mesos">&lsaquo;</abbr></div>' +
  ...
  Calendar.prototype.render = function () {
    ...
    var row;
    this.model.getMonth().forEach(function (day, i, days) {
      ...
      dayEl.setAttribute("data-year", self.model.date.year);
...
module.exports = (function () {
  ...
  function DateTime(onChange, config) {
    ...
    this.onChange = function (year, month, day) {
      if (!self.config.background) {
        Array.apply(null, self.calendarView.el.getElementsByClassName("day")).map(
          function (el) {
            if (
              el.getAttribute("data-year") == year &&
              el.getAttribute("data-month") == month - 1 &&
    ...
    this.timeLineView = new TimeLineView(console.log, this.config);
    ...
    if (!this.config.background) {
      ...
      this.calendarView.el.addEventListener("change", function (ev) {
        self.onChange(
          ...
          self.calendarView.model.date.day,
  ...
  DateTime.prototype.start = function start() {
    ...
    function next() {
      ...
      new Promise(function (res, rej) {
        ...
        self
          .onChange(date.year, Number(date.month) + 1, date.day + 1, hour)
          .then(function () {
            if (self.config.background) {
              next();
            } else {
